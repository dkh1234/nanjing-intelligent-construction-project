...
def parse_one_report(text, filename=""):
    ...
    # 工人: 优先取 "总人数" 模式，否则匹配所有 "xx人"
    total_match = re.search(r"总人数\s*(\d+)\s*人", full)
    if total_match:
        rec["total_workers"] = int(total_match.group(1))
    else:
        worker_nums = re.findall(r"(\d+)\s*人", full)
        if worker_nums:
            rec["total_workers"] = sum(int(n) for n in worker_nums)

    # 分包商数量: "xx：\d+人" 模式（如"中建八局：343人"）
    sub_matches = re.findall(r"[：:]\s*(\d+)\s*人", full)
    rec["num_subcontractors"] = len(sub_matches)
